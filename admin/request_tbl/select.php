...
mysqli_query($con,"update $table set status='accepted' where id='$up_id'");
//if($del_id!="")
}	
...
mysqli_query($con,"update $table set status='rejected' where id='$del_id'");
//if($del_id!="")
}
	?>
    <script>
   ...
   echo "<tbody>";
 	...
 	$result = mysqli_query($con,"SELECT * FROM $table ");
		...
		for($k=0;$k<$i;$k++)
		{
			...
			if($k==1)
			{
			  $sql2 = "select *  from user where id='$row[$k]' ";
...
$row2 =mysqli_fetch_array($result2);
			...
			<td><a href='?up_id=$id'>Accept</a></td>
			
			<td><a href='?del_id=$id' onclick='return rem()'>Reject</a></td>
